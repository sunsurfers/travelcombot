...
def start_command_handler(message):
    ...
    if not user:
        logging.info('Started by {!s}, id {!s}'.format(
            message.from_user.first_name, cid))
...
def location_content_handler(message):
    ...
    if uid in READY_TO_SHARE_LOCATION:
        ...
        coordinates = '{!s} {!s}'.format(
            message.location.longitude, message.location.latitude)
        database.add_visited_place(
            user['id'], coordinates, '', datetime.datetime.now())
...
def text_content_handler(message):
        ...
        if 'name' not in READY_TO_REGISTER[uid]:
            ...
            if message.text == 'Другое':
                ...
                keyboard.add(types.InlineKeyboardButton(
                    'Узнать, кто такие сансерферы', url='http://sunsurfers.ru'))
                keyboard.add(types.InlineKeyboardButton(
                    'Узнать, кто такие сменщики', url='http://smenastation.com'))
            ...
            text = 'Ты выбрал сообщество {!s}'.format(
            ...
            for x in typeofevents:
                keyboard.add(types.InlineKeyboardButton(
                    text=x['name'], callback_data='selecttypeofevent_{!s}'.format(x['id'])))
            ...
            keyboard = types.InlineKeyboardMarkup()
            keyboard.add(types.InlineKeyboardButton(
                '✅ Подтвердить', callback_data='confirmanket_{!s}_{!s}'.format(user['id'], user['telegram'])))
            keyboard.add(types.InlineKeyboardButton(
                '❌ Отклонить', callback_data='refuseanket_{!s}_{!s}'.format(user['id'], user['telegram'])))
            bot.send_photo(config.admin_channel_id, open(READY_TO_REGISTER[uid]['avatar'], 'rb'),
                           caption=channel_text, reply_markup=keyboard, parse_mode='HTML')
    ...
    if uid in READY_TO_ADMIN_EMAIL:
        if 'text' not in READY_TO_ADMIN_EMAIL[uid]:
            ...
            for x in users:
                try:
                    bot.send_message(
                        x['telegram'], READY_TO_ADMIN_EMAIL[uid]['text'])
    ...
    if uid in READY_TO_ADD_INSTA:
        if 'text' not in READY_TO_ADD_INSTA[uid]:
            ...
            READY_TO_ADD_INSTA[uid]['text'] = 'https://instagram.com/{!s}'.format(
                message.text[1:])
    ...
    if message.text == 'ℹ️ Рассказать о себе и своих интересах':
        ...
        for x in typeofevents:
            keyboard.add(types.InlineKeyboardButton(
                text=x['name'], callback_data='addselecttypeofevent_{!s}'.format(x['id'])))
        return bot.send_message(cid, texts.register_type_events_question_text, reply_markup=keyboard)
        ...
        text = '{!s}\nТвой инстаграм на данный момент: {!s}\n\nПример: @seva.randev'.format(
            texts.add_insta, insta_text)
        return bot.send_message(cid, text, reply_markup=markup)
...
def callback_inline(call):
    ...
    if call.data.startswith('selecttypeofevent'):
        ...
        for x in events:
            ...
            event_name = '{!s}, {!s}, {!s} {!s}'.format(
                x['name'], country['name'], month_name, x['date_of_the_event'].year)
            keyboard.add(types.InlineKeyboardButton(
                text=event_name, callback_data='selectevent_{!s}'.format(x['id'])))
        keyboard.add(types.InlineKeyboardButton(
            text='↪️ Назад', callback_data='selectalltypeevents'))
        return bot.edit_message_text(texts.register_events_question_text, chat_id=cid,
        ...
        bot.edit_message_text(
            text, chat_id=cid, message_id=call.message.message_id, reply_markup=None)
        text = 'Желаешь отметить ещё мероприятия?'
        keyboard = types.InlineKeyboardMarkup()
        keyboard.add(types.InlineKeyboardButton(
            text='➕ Добавить ещё', callback_data='selectalltypeevents'))
        keyboard.add(types.InlineKeyboardButton(
            text='➡️ Далее', callback_data='nextactionsafterevents'))
        ...
        for x in typeofevents:
            keyboard.add(types.InlineKeyboardButton(
                text=x['name'], callback_data='selecttypeofevent_{!s}'.format(x['id'])))
        return bot.edit_message_text(texts.register_type_events_question_text, chat_id=cid,
                                     message_id=call.message.message_id, reply_markup=keyboard)
    ...
    if call.data.startswith('addselecttypeofevent'):
        ...
        events = database.get_events_by_event_type_id(typeofevent_id)
        user = database.get_user(uid)
        user_event_ids = [x['event']
                          for x in database.get_user_events(user['id'])]
        keyboard = types.InlineKeyboardMarkup()
        for x in events:
            ...
            event_name = '{!s}, {!s}, {!s} {!s}'.format(
                x['name'], country['name'], month_name, x['date_of_the_event'].year)
            keyboard.add(types.InlineKeyboardButton(
                text=event_name, callback_data='addselectevent_{!s}'.format(x['id'])))
        keyboard.add(types.InlineKeyboardButton(
            text='↪️ Назад', callback_data='addselectalltypeevents'))
        return bot.edit_message_text(texts.register_events_question_text, chat_id=cid,
                                     message_id=call.message.message_id, reply_markup=keyboard)
        ...
        keyboard = types.InlineKeyboardMarkup()
        keyboard.add(types.InlineKeyboardButton(
            text='➕ Добавить ещё', callback_data='addselectalltypeevents'))
        return bot.send_message(cid, text, reply_markup=keyboard)
